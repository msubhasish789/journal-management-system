            ...
            <div>
                <?php
                        ...
                        FROM `Manuscript Submission` ms JOIN `author` a ON ms.`Author id` = a.`Author id`
                                WHERE ms.`Manuscript id` in (SELECT `Manuscript ID` FROM `review_request_temp` WHERE `Reviewer ID`= '$reviewer_id');";
                ...
                <div class="manuscript_list">
                    ...
                    <div>
                        <a href="reject_review_request.php?manuscriptId=<?php echo $arr["Manuscript id"]; ?>&reviewerID= <?php echo $reviewer_id; ?>" class="btn reject_btn" id="<?php  $arr["Manuscript id"]; ?>">Reject</a>
                        <a href="accept_review_request.php?manuscriptId=<?php echo $arr["Manuscript id"]; ?>&reviewerID= <?php echo $reviewer_id; ?> " class="btn accept_btn" id="<?php  $arr["Manuscript id"]; ?>">Accept</a>
                        <a href="view_manuscript.php?manuscriptId=<?php echo $arr["Manuscript id"]; ?>" class="btn" id="<?php  $arr["Manuscript id"]; ?>">View</a>
                    </div>
                </div>

                <?php }
                }else echo "<h2> No Pending Review Request </h2>" ?>
